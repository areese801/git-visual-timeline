...
class FlashAuthorModal(ModalScreen):
    """Shows who last modified the file and a contributor breakdown."""
    ...
    # Rotating colors for the bar chart
    BAR_COLORS = [COLOR_ACCENT, COLOR_PURPLE, COLOR_GREEN, COLOR_AMBER, COLOR_RED]
    ...
    def compose(self) -> ComposeResult:
        ...
        text.append("Last modified by  ", style=COLOR_DIM)
        text.append(f"{self.author}\n", style=f"bold {COLOR_ACCENT}")
        text.append(f"{self.short_hash}  {self.date.strftime('%Y-%m-%d %H:%M')}\n\n", style=COLOR_DIM)

        # Contributor breakdown
        total = sum(count for _, count in self.contributors)
        if total == 0:
            text.append("No contributors found", style=COLOR_DIM)
        else:
            text.append("Contributors\n", style=f"bold {COLOR_DIM}")

            bar_width = 40

            # Stacked bar — one line showing proportional segments
            text.append("  ")
            for i, (author, count) in enumerate(self.contributors):
            ...
            for i, (author, count) in enumerate(self.contributors):
                ...
                text.append(f" {author}", style=COLOR_TEXT)
                text.append(f"  {count} commits ({pct:.0f}%)\n", style=COLOR_DIM)

        text.append(f"\n  Press any key to close", style=COLOR_DIM)

        with Container(id="flash-author-container"):
            yield Static(text)
